...
static void dump_column_content_from_tail(
    column_t *column_src,
    ...
    size_t nb_rows)
{
    ...
    if (!nb_rows)
        return;
    for (size_t i = (size_t)nb_rows - 1; i >= nb_rows - expected_rows; i--) {
        column_dest->content_strings[e] =
            strdup(column_src->content_strings[i]);
        column_dest->content[e] = column_src->content[i];
        e++;
        if (!i)
            return;
    }
...
static column_t **dump_columns_from_tail(
    dataframe_t *df_src,
    dataframe_t *df_dest)
{
    size_t i = 0;
    ...
    while (i < df_src->nb_columns) {
        ...
        dump_column_content_from_tail(df_src->columns[i], data[i],
            df_dest->nb_rows, df_src->nb_rows);
        i++;
    }
    return data;
}

static dataframe_t *dump_dataframe_from_tail(
    dataframe_t *df_src,
    size_t nb_rows)
{
    ...
    new_dataframe->columns = dump_columns_from_tail(df_src, new_dataframe);
    ...
    int nb_rows)
{
    ...
    if (!dataframe || nb_rows < 0)
        return NULL;
    new_dataframe = dump_dataframe_from_tail(dataframe, (size_t)nb_rows);
    return new_dataframe;
}
